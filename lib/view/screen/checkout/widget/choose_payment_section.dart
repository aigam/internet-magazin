...
class ChoosePaymentSection extends StatelessWidget {
  ...
  const ChoosePaymentSection({Key? key, required this.onlyDigital})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Consumer<OrderProvider>(builder: (context, orderProvider, _) {
      return Consumer<SplashProvider>(builder: (context, configProvider, _) {
        var pays = configProvider.configModel!.paymentMethods ?? [];
          ...
          pays = [
            PaymentMethods(
                ...
                additionalDatas: AdditionalDatas(
                    gatewayTitle: "Тинькоф", gatewayImage: "tinkoff.svg"))
          ];

          WidgetsBinding.instance.addPostFrameCallback((_) {
            orderProvider.setOfflineChecked('tinkof');
          });
        }
                        ...
                        ? Row(
                            children: [
                              SizedBox(
                                  ...
                                  child: CustomImage(
                                      image:
                                          '${configProvider.configModel?.paymentMethodImagePath}/${pays[orderProvider.paymentMethodIndex].additionalDatas!.gatewayImage ?? ''}')),
                              Padding(
                                padding: const EdgeInsets.symmetric(
                                    horizontal: Dimensions.paddingSizeSmall),
                                child: Text(
                                    pays[orderProvider.paymentMethodIndex]
                                            .additionalDatas!
                                            .gatewayTitle ??
                                        ''),
                              ),
